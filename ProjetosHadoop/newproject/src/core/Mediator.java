...
public class Mediator {
     ...
     */
    public static void readClassifierConfiguration (Configuration conf) throws Base64DecodingException, IOException, ClassNotFoundException{
    	...
    	String frmStr = conf.get(FRM_FIELD);
    	...
    	numLinguisticLabels = Byte.parseByte(conf.get(NUM_LINGUISTIC_LABELS_FIELD));
     ...
     * @throws ClassNotFoundException 
     */
    public static void readLearnerConfiguration (Configuration conf) throws Base64DecodingException, IOException, ClassNotFoundException{
    	
    	String buffer;
    	
    	buffer = conf.get(COST_SENSITIVE_FIELD).toLowerCase().trim();
    	...
    	learnerRuleBaseSize = Integer.parseInt(conf.get(LEARNER_RULE_BASE_SIZE_FIELD));
    	hdfsLocation = conf.get(HDFS_LOCATION_FIELD);
    	numLinguisticLabels = Byte.parseByte(conf.get(NUM_LINGUISTIC_LABELS_FIELD));
    	numClassLabels = Byte.parseByte(conf.get(NUM_CLASSES_FIELD));
    	numVariables = Integer.parseInt(conf.get(NUM_VARIABLES_FIELD));
    	
    	// Read class labels
    	byte[] bytes = Base64.decode(conf.get(CLASS_LABELS_FIELD).getBytes());
    	ObjectInputStream objectInputStream = new ObjectInputStream(new ByteArrayInputStream(bytes));
    	classLabels = (String[])objectInputStream.readObject();
    	
    	// Read variables
    	bytes = Base64.decode(conf.get(VARIABLES_FIELD).getBytes());
     ...
     * @throws URISyntaxException 
     */
    public static void storeChiParameters (String inputFilePath) throws IOException, URISyntaxException{
